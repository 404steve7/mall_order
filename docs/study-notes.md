...
- `@RocketMQMessageListener`：声明 RocketMQ 消费者，监听指定 Topic 的消息。
...
Redis 是一个独立运行的内存数据库。当前项目通过 Docker 启动 Redis，并让 Spring Boot 连接本机：

```text
localhost:6379
```

本项目第一阶段已经把 Redis 用在商品详情缓存上。

```text
第一次查商品：Redis 没有 -> 查 MySQL -> 写入 Redis
第二次查商品：Redis 有 -> 直接从 Redis 返回
修改商品：更新 MySQL -> 删除对应 Redis 缓存
```
...
Redis 中保存的是字符串，所以后端会先把 `Product` 对象转成 JSON 字符串再存进去。当前使用的是：
...
`StringRedisTemplate` 负责读写 Redis 字符串，`ObjectMapper` 负责 Java 对象和 JSON 字符串互相转换。
...
`miss` 表示 Redis 没有缓存，需要查 MySQL。`hit` 表示 Redis 有缓存，可以直接返回缓存内容。
...
这个过期时间也叫 TTL。TTL 的作用是避免缓存长期保存旧数据。

为什么修改商品后要删除缓存：

```text
商品详情缓存里保存的是旧商品信息。
如果修改商品后不删缓存，下一次查询可能仍然读到旧数据。
所以更新 MySQL 成功后，要删除 product:detail:{id}。
下一次查询时重新查 MySQL，再写入新的缓存。
```

已验证场景：

```text
GET /product/4 后 Redis 出现 product:detail:4
再次 GET /product/4 出现 product cache hit
PUT /product/4 后 product:detail:4 变成 nil
...
```text
Apifox 调 POST /order/create
↓
OrderController
↓
OrderService 创建订单、扣库存、写 MySQL
↓
OrderMessageProducer 发送订单号
↓
RocketMQ Broker 保存并投递消息
↓
OrderMessageConsumer 收到消息
↓
控制台打印 receive order created message
```
...
为什么不用 Controller 直接发消息：

```text
Controller 负责接收 HTTP 请求。
Service 负责业务流程。
OrderMessageProducer 专门负责发送消息。
这样代码职责更清楚，以后消息发送方式变化时，不需要把 Controller 搅乱。
```
...
```text
消息的含义是“订单已创建”。
如果数据库还没写成功就先发消息，消费者可能收到一个并不存在的订单号。
所以要在订单主表和明细表插入成功后再发送。
```

当前消费者只打印日志，这是学习版做法。真实项目里消费者可能会继续做：
...
```text
RocketMQ、Redis、MySQL 都是外部服务。
现在运行测试前，这些服务都需要启动。
如果 RocketMQ 没启动，Spring Boot 初始化 Producer 或 Consumer 时可能失败，测试会变慢或失败。
```
...
- 已启动 RocketMQ NameServer 和 Broker。
- 已完成 Spring Boot 接入 RocketMQ。
- 已完成下单成功后发送订单消息。
- 已完成消费者接收消息并打印日志。
- 已通过 Apifox 验证创建订单后出现消费日志。
- 回归测试、整理文档、提交代码。
